...
def main():
    ...
    write_species_map_to_dynamo(csvfile_species_mapping)
    write_species_map_to_dynamo(csvfile_label_mapping)
    return print("Done")
...
def write_species_map_to_dynamo(keyVal):
    table = dynamodb.Table(tableName)
    with table.batch_writer() as batch:
        for species in keyVal:
            for label in keyVal[species]:
                baseLabel = re.sub("LABEL-","",label)
                batch.put_item(
                    Item={
                        'PartitionKey': "SPECIES-"+species,
                        'SortKey': label,
                        'level3': [baseLabel]
                    }
                )
...
def write_label_map_to_dynamo(keyVal):
    table = dynamodb.Table(tableName)
    with table.batch_writer() as batch:
        for label in keyVal:
